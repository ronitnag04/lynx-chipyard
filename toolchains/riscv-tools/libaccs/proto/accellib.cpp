...
volatile char ** AccelSetupFixedAllocRegionSerializer() {
    ROCC_INSTRUCTION(PROTOACC_SER_OPCODE, FUNCT_SER_SFENCE+SER_EXTEND);
    ...
    ROCC_INSTRUCTION_SS(PROTOACC_SER_OPCODE, stringalloc_region_ptr_as_int_tail, string_ptr_region_ptr_as_int, FUNCT_SER_MEM_SETUP+SER_EXTEND);
...
volatile char * BlockOnSerializedValue(volatile char ** ptrs, int index) {
    uint64_t retval;
    printf("Fn 4: wait for return value\n");
    ROCC_INSTRUCTION_D(PROTOACC_SER_OPCODE, retval, FUNCT_SER_CHECK_COMPLETION+SER_EXTEND);
    asm volatile ("fence");
...
void AccelParseFromString_Helper(const void * descriptor_table_ptr, void * dest_base_addr,
    ...
    printf("Fn 1: Descriptor address, Destination address\n");
    ROCC_INSTRUCTION_SS(PROTOACC_OPCODE, descriptor_table_ptr, dest_base_addr, FUNCT_PROTO_PARSE_INFO+DES_EXTEND);
    printf("Fn 2: Base ptr (=input string's pointer), Min field # and input length\n");
    ROCC_INSTRUCTION_SS(PROTOACC_OPCODE, base_ptr, min_field_no_and_input_length, FUNCT_DO_PROTO_PARSE+DES_EXTEND);
    printf("Deserializer action start\n");

}

uint64_t block_on_completion() {
    uint64_t retval;
    ROCC_INSTRUCTION_D(PROTOACC_OPCODE, retval, FUNCT_CHECK_COMPLETION+DES_EXTEND);
    asm volatile ("fence");
    return retval;
}
...
void AccelSerializeToString_Helper(const void * descriptor_table_ptr, void * src_base_addr) {
    ...
    ROCC_INSTRUCTION_SS(PROTOACC_SER_OPCODE, hasbits_offset, min_max_fieldno, FUNCT_HASBITS_INFO+SER_EXTEND);
    ROCC_INSTRUCTION_SS(PROTOACC_SER_OPCODE, descriptor_table_ptr, src_base_addr, FUNCT_DO_PROTO_SERIALIZE+SER_EXTEND);
}
