...
int rqueue_peek_check(rqueue_t *rb, uint64_t iv) {
    int i;
    bool rc = false;
    void *v = NULL;
    ...
    for (i = 0; i < RQUEUE_MAX_RETRIES; i++) {
        ...
        if (__builtin_expect(ATOMIC_CAS(rb->read_sync, 0, 1), 1)) {
            ...
            v = ATOMIC_READ(head->value);
            if (((rqueue_elem_t*)v)->metadata == iv) {
                ATOMIC_CAS(rb->read_sync, 1, 0);
                return true;
            } else {
                ATOMIC_CAS(rb->read_sync, 1, 0);
           ...
           /* TODO - FIXME
            ...
            if (rb->mode == RQUEUE_MODE_OVERWRITE && ATOMIC_READ(commit->next) == temp_page) {  // DELETE ME WHEN FIXED
               if (ATOMIC_CAS(rb->read_sync, 0, 1)) { // TODO - it shouldn't be necessary to synchronize with the reader
                    ...
                    //printf("Will advance head and overwrite old data\n");
...
size_t rqueue_current_size(rqueue_t *rb) {
    return rqueue_write_count(rb) - rqueue_read_count(rb);
}
